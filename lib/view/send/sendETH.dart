...
class Sendeth extends ConsumerStatefulWidget {
  const Sendeth({super.key});
  static const routeName = '/sendeth';

  @override
  ConsumerState<Sendeth> createState() => _SendethState();
}

class _SendethState extends ConsumerState<Sendeth> {
  ...
  Future<void> _verify() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (!_hasEnoughBalance(amount, user!.ETH)) {
        showMessage(
          context,
          'You don\'t have sufficient amount of ETH for this transaction',
        );
        return;
      }
      _startLoading();
  ...
  Future<void> _processWithdrawal(User user, double amount) async {
    ...
    showMessage(
      context,
      'Your ETH Withdrawal is processing, check Transaction history to confirm status',
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Send Ethereum'),
      ),
      body: Padding(
  ...
  void _updateAmount(double percentage) {
    final user = ref.read(authProvider).user;
    final amount = user!.ETH * percentage;
